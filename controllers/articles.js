...
const createArticle = (req, res, next) => {
  ...
  Article.create({
    ...
    .then((article) => {
      if (!article) { throw new RequestError('Ошибка создания карточки статьи'); }
      res.send(article);
    })
    .catch(next);
};
...
const deleteArticleById = (req, res, next) => {
  ...
  // eslint-disable-next-line consistent-return
    .then((article) => {
      ...
      if (JSON.stringify(article.owner._id) !== JSON.stringify(user)) { return Promise.reject(new AuthError('Вы не являетесь автором карточки статьи')); }
